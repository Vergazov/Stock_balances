...
class JsonApi {
    ...
    function allStores($filter = null) {      
        
        if($filter != null){
            $result =  makeHttpRequest(
                'GET',
                cfg()->moyskladJsonApiEndpointUrl . '/entity/store' . '?' . $filter,
                $this->accessToken);
                return $result->rows;
        }else{
            $result =  makeHttpRequest(
    ...
    function getStocksByStore($href) {
        $result =  makeHttpRequest(
            'GET',
            cfg()->moyskladJsonApiEndpointUrl . "/report/stock/all?filter=store=$href",
            $this->accessToken);
    ...
    function buildTree(array $elements, $parentId = 0) {
        ...
        foreach ($elements as $element) {
            if ($element['parent_id'] === $parentId) {
                $child = $this->buildTree($elements, $element['id']);
                if ($child) {
                    $element['child'] = $child;
                }
                $branch[] = $element;        
